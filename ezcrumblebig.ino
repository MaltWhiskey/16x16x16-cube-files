...
int axis = 1;		// Which axis are we crumbling through?  0=x, 1=y, 2=z.
bool mirror2 = true;
std::vector< int > remaining;
int pick, offset2, flips;
const int NUM_FLIPS = 5;
Color clearColor = Color( 5, 0, 0 );
Color mainColor = Color( 0, 0, 50 );
...
// This method first applies the transform to rotate the whole thing on its x, y, or z axis, and mirror the whole animation.
void setVoxel2( int x, int y, int z, bool clear ){
	if( mirror2 )
		z = 15-z;
	...
	setVoxel( x, y, z, clear?clearColor:mainColor );
}
...
void resetCycle(){
	...
	for( int x=0; x<16; x++ )
		for( int y=0; y<16; y++ )
			for( int z=0; z<16; z++ )
				setVoxel2( x, y, z, true );

	for( int i=0; i<256; i++ ){
		remaining.push_back( i );
		int x = i/16;
		int y = i%16;
		setVoxel2( x, y, 0, false );
	}
	show();
}

// Each time it's called, shifts the currently picked voxel and shifts it by one
// space.  It returns false once it reaches the other wall.
bool shift(){
	int x = pick/16;
	int y = pick%16;

	setVoxel2( x, y, offset2, false );
	if( offset2>0 )
		setVoxel2( x, y, offset2-1, true );
	show();

	return ++offset2<16;
}

// Selects the next voxel to shift, by drawing it from the remaining collection.
// When no voxels remain, it "flips" and resets the cycle
int draw(){
