//This compiles and runs on Particle Build and 16X16X16 cube
...
bool demo2;          //Use this to enable/disable the demo sequence playback 
    ...
    for(int y=-radius; y<=radius; y++)
        for(int x=-radius; x<=radius; x++)
            if(x*x+y*y <= radius*radius)
                setVoxel(xOrigin+x, z, yOrigin+y, col);
}
 //Checks to see if it's any gradient of white
        bool isWhiteColor(Color col) {
	return (col.red == col.green && col.red == col.blue && col.red > 64);
        }
	//Checks to see if it's black
       bool isBlackColor(Color col) {
           return (col.red == col.green && col.red == col.blue && col.red <1);
...
void christmasTree() {
    ...
    //if(isFirstLap) {
        //Paint the tree
       for(int z=0;z<13;z+=2,radius--) {
            int gradient = SIZE-(radius)*(z*(radius));
            //Color gradedGreen = {fadeSqRt(green.red+gradient),fadeLinear(green.green+gradient),fadeSquare(green.blue+gradient)};
            drawSolidHorizontalCircle(xOrigin, yOrigin, z, radius, Color(0,10,0));
            drawSolidHorizontalCircle(xOrigin, yOrigin, z+1, radius-2, Color(0,10,0));
        ...
        //make the star
        setVoxel(xOrigin,15,yOrigin,Color(80,80,0));
        delay(100);
        setVoxel(xOrigin,15,yOrigin,Color(0,0,0));
        delay(100);
        setVoxel(xOrigin,15,yOrigin,Color(80,80,0));
    ...
    //draw lights?
       //drawing a friggin' inverse conical helix with just a friggin' 8-bit 3D resolution is a friggin' PAIN!!
       // Color col;
        radius = 8;
        for(int z=0;z<15;z++) {
            for(int d=0; d<=(2*PI); d++)
                if(z%2>0) {
                    ...
                    setVoxel(xOrigin+((radius-.75)*cos(d)), z, yOrigin+((radius-.75)*sin(d)), Color(rand()%100, rand()%100,rand()%100));
                }
                else {
                    //randomColor(&col);
                    setVoxel(xOrigin+(radius*cos(d)), z, yOrigin+(radius*sin(d)), Color(rand()%150, rand()%150,rand()%150));    
                }
    ...
    //make it snow?
{
        ...
        for(int x=0;x<16;x++) {
            for(int y=0;y<16;y++) {
                for(int z=0;z<16;z++) {
                    if (isWhiteColor(getVoxel(x,y,z))) {
                        setVoxel(x,y,z,Color(0,0,0));  // If the voxel is white turn it black
                        if(isBlackColor(getVoxel(x,y,z))) {
                            ...
                            whiteLevel = random(65,66);
                            flakeColor = {whiteLevel, whiteLevel, whiteLevel};
                            setVoxel(x,y-1,z,flakeColor);  
                        }
                    }
                }
            }
        ...
        for(uint16_t i=0;i<numFlakes;i++) {
            ...
            do{
                flake.x = random(0,15);
                flake.z = random(0,15);
            } while (!isBlackColor(getVoxel(flake.x, 15, flake.z)));
            //make a flake
            whiteLevel = random(65,66);
            flakeColor = {whiteLevel, whiteLevel, whiteLevel};
            setVoxel(flake,flakeColor);
        }
    }
